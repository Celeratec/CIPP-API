function Push-BECRun {
    ...
    try {
        ...
        Write-Information 'Getting trusted/blocked senders'
        # Manage365 addition: upstream's BEC UI has a Trusted & Blocked Senders check but no
        # producer for these fields yet; we collect them from the junk email configuration.
        try {
            ...
            $TrustedSenders = @()
            $BlockedSenders = @()
            $CippJunkError = Get-CippException -Exception $_
            Write-LogMessage -API 'BECRun' -message "Failed to retrieve junk email configuration for $($UserName): $($CippJunkError.NormalizedError)" -tenant $TenantFilter -sev Warning -LogData $CippJunkError
        }
        ...
        try {
            $SafelistChanges = if ($auditLog -eq $false) { @() } else {
                ...
                (New-ExoRequest -tenantid $TenantFilter -cmdlet 'Search-UnifiedAuditLog' -cmdParams $SafelistSearchParam -Anchor $UserName).AuditData | ConvertFrom-Json -ErrorAction Stop |
                    Where-Object { $_.UserId -eq $UserName -or $_.MailboxOwnerUPN -eq $UserName -or $_.ObjectId -like "*$UserName*" } | ForEach-Object {
                        [pscustomobject]@{
                            Operation = $_.Operation
                            UserKey   = $_.UserId
                            Date      = $_.CreationTime
                            Trusted   = ($_.Parameters | Where-Object { $_.Name -eq 'TrustedSendersAndDomains' }).Value
                            Blocked   = ($_.Parameters | Where-Object { $_.Name -eq 'BlockedSendersAndDomains' }).Value
                        }
                    }
            }
        } catch {
            ...
            Write-LogMessage -API 'BECRun' -message "Failed to retrieve safelist changes for $($UserName): $($CippSafelistError.NormalizedError)" -tenant $TenantFilter -sev Warning -LogData $CippSafelistError
        }
