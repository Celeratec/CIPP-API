function Invoke-CIPPStandardintuneDeviceRetirementDays {
    ...
    $TestResult = Test-CIPPStandardLicense -StandardName 'intuneDeviceRetirementDays' -TenantFilter $Tenant -Preset Intune
    ...
    if ($Settings.report -eq $true) {
        $CurrentValue = @{
            deviceInactivityBeforeRetirementInDays = $CurrentInfo.DeviceInactivityBeforeRetirementInDays
        }
        $ExpectedValue = @{
